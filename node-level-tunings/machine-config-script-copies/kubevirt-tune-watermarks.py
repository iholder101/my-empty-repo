...
KubeletParams = namedtuple("KubeletParams", "eviction_kb sys_reserved_kb kube_reserved_kb")
...
def get_kubelet_params(total_ram_kb):
    """Read eviction threshold and reserved memory from kubelet config."""
    ...
    # Eviction threshold
    raw_eviction = data.get("evictionHard", {}).get("memory.available", "")
    if raw_eviction:
        print(f"Eviction threshold from config: {raw_eviction}")
        eviction_kb = parse_memory_kb(raw_eviction, total_ram_kb)
    else:
        print("Eviction threshold not found in config, using default: 100Mi")
        eviction_kb = DEFAULT_EVICTION_KB

    # System reserved memory
    raw_sys = data.get("systemReserved", {}).get("memory", "")
    if raw_sys:
        try:
            sys_reserved_kb = parse_memory_kb(raw_sys, total_ram_kb)
            print(f"systemReserved.memory from config: {raw_sys} ({sys_reserved_kb // 1024} MiB)")
        except SystemExit:
            print(f"WARNING: could not parse systemReserved.memory '{raw_sys}', assuming 0")
            sys_reserved_kb = 0
    else:
        sys_reserved_kb = 0

    # Kube reserved memory
    raw_kube = data.get("kubeReserved", {}).get("memory", "")
    if raw_kube:
        try:
            kube_reserved_kb = parse_memory_kb(raw_kube, total_ram_kb)
            print(f"kubeReserved.memory from config: {raw_kube} ({kube_reserved_kb // 1024} MiB)")
        except SystemExit:
            print(f"WARNING: could not parse kubeReserved.memory '{raw_kube}', assuming 0")
            kube_reserved_kb = 0
    else:
        kube_reserved_kb = 0

    return KubeletParams(eviction_kb, sys_reserved_kb, kube_reserved_kb)
...
def main():
    ...
    kp = get_kubelet_params(total_ram_kb)
    eviction_kb = kp.eviction_kb
    reserved_kb = kp.sys_reserved_kb + kp.kube_reserved_kb
    ...
    if reserved_kb:
        print(f"Reserved memory (system + kube): {reserved_kb // 1024} MiB")
    ...
    target_low_kb = eviction_kb + runway_kb + reserved_kb
